#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# PumpHunter v4.1 - Full corrected script (ready to paste & run)
# - Early-detection, multi-threaded scanning, formatted telegram messages
...
# PumpHunter v4.1 - Full corrected script (ready to paste & run)
# - Early-detection, multi-threaded scanning, formatted telegram messages
# - Added: TP follow-up notifications + 5% fixed stop loss (as requested)
...
# ---------------- CONFIG ----------------
DRY_RUN = os.environ.get("DRY_RUN", "0") != "0"   # safe default: DRY_RUN OFF if not set to "1"
...
SCORE_ALERT = int(os.environ.get("SCORE_ALERT", 70))
PRE_SIGNAL = int(os.environ.get("PRE_SIGNAL", 70))
...
# ---------------- Telegram ----------------
def tg_send(msg):
    # Preview log (single-line for logs)
    try:
        log.info("TG PREVIEW: %s", msg.replace("\n", " | ")[:500])
    except Exception:
        log.info("TG PREVIEW (short): %s", msg[:300])
    if DRY_RUN:
        return True
...
# Create nicely formatted message exactly like requested
CIRCLED = ['①','②','③','④','⑤','⑥','⑦']
...
def publish_trade(sym, features, reasons, price, score):
    ...
    # Put green or red emoji depending on side
            ...
            for idx, tp in enumerate(tps):
                if not hit[idx]:
                    if (side == 'LONG' and price >= tp) or (side == 'SHORT' and price <= tp):
                        ...
                        # New TP message format requested
...
def scan_symbol(sym):
    try:
        ...
        return {'symbol': sym, 'features': features, 'score': score, 'reasons': reasons, 'pulse': pulse_vals, 'last': features['last']}
    except Exception as e:
